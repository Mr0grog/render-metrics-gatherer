  ...
  for (const event of events) {
    ...
    if (runId) {
      ...
      if (event.__typename === 'CronJobRunStarted') {
        run.queueTime = timestamp;
      } else if (event.__typename === 'CronJobRunEnded') {
        // Sometimes there are multiple job ended events for failures. Do our
        // best to sort out the most accurate info.
        if (!run.endTime || run.endTime > timestamp) {
          run.endTime = timestamp;
        }
        run.status = Math.max(run.status || 0, event.status);
